...
Before choosing a resolution, save the local trunk, ordered branch names, and
heads from `gh stack view --json` and inspect the remote composition. Keep
unpublished work recoverable. Removing tracking must not silently discard the
user's chosen stack structure; apply only the resolution covered by the task.
...
- **Keep the local version.** Remove the grouping on GitHub, then rebuild local
  tracking using the saved trunk and branch order before submitting.

  ```bash
  gh stack unstack                  # removes the grouping; PRs and branches survive
  gh stack init --base <saved-trunk> <saved-bottom-branch> <saved-next-branch>
  gh stack submit --auto
  ```

Neither path deletes pull requests or branches.
Remote unstacking leaves PRs that are merging (auto-merge enabled) or are queued
(in a merge queue) stacked. Report that state; change it only if the task covers
cancelling auto-merge or queue membership.
...
Another `gh stack` process holds the exclusive lock on `.git/gh-stack.lock`. The
lock times out after about five seconds, so wait briefly and retry once. For a
persistent exit 8, identify the process and report the contention; do not stop
someone else's active operation or remove its lock.
